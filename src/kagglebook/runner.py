...
class Runner:
    ...
    def run_train_cv(self) -> None:
        ...
        # 予測結果の保存
        Util.dump(preds, config.OOF_DIR / f"{self.run_name}-train.pkl")
    ...
    def run_predict_cv(self) -> None:
        ...
        # 予測結果の保存
        Util.dump(pred_avg, config.PRED_DIR / f"{self.run_name}-test.pkl")
    ...
    def run_predict_all(self) -> None:
        ...
        # 予測結果の保存
        Util.dump(pred, config.PRED_DIR / f"{self.run_name}-test.pkl")
    ...
    def load_x_train(self) -> pd.DataFrame:
        ...
        # 毎回train.csvを読み込むのは効率が悪いため、
        #  データに応じて適宜対応するのが望ましい（他メソッドも同様）
        return pd.read_csv(config.RAW_DATA_DIR / "train.csv")[self.features]
    ...
    def load_y_train(self) -> pd.Series:
        ...
        # 目的変数の読込を行う
        train_y = pd.read_csv(config.RAW_DATA_DIR / "train.csv")[TARGET_COL]
    ...
    def load_x_test(self) -> pd.DataFrame:
        ...
        :return: テストデータの特徴量
        """
        return pd.read_csv(config.RAW_DATA_DIR / "test.csv")[self.features]
